...
async def record_session_engagement(*, user_id, session_number):
    if not DB_ENABLED:
        return {"persisted": False}
    parts = ist_now_parts()
    result = await query(
        """INSERT INTO session_count (user_id, session_number, log_date, log_time)
           VALUES (%s, %s, %s, %s)
           RETURNING id, user_id, session_number, log_date, log_time""",
        [user_id, session_number, parts["date"], parts["time"]],
    )
